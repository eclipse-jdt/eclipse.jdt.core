...
public final class ImportRewriteAnalyzer {
	...
	private static class PackageMatcher {
		...
		public PackageMatcher() {
			// initialization in initialize(String, String)
		}
		
		public void initialize(String newNameInit, String bestNameInit) {
			this.newName= newNameInit;
			this.bestName= bestNameInit;
			this.bestMatchLen= getCommonPrefixLength(bestName, this.newName);
		}
	...
	final protected static int getCommonPrefixLength(String s, String t) {
	...
	final protected static char getCharAt(String str, int index) {
		if (str.length() > index) {
	...
	/*
	 * Probes if the formatter allows spaces between imports
	 */
	...
	private static final class ImportDeclEntry {
		...
		public int compareTo(String fullName, boolean isStatic) {
			...
			if (cmp == 0) {
				if (this.isStaticImport == isStatic) {
					return 0;
				}
				return this.isStaticImport ? -1 : 1;
			}
			return cmp;
		}
	...
	private final static class PackageEntry {
		...
		public PackageEntry(String name, String group, boolean isStatic) {
			...
			this.isForStatic= isStatic;
		}
		
		public boolean isStatic() {
			return this.isForStatic;
		}
		
		public int compareTo(String name, boolean isStatic) {
			int cmp= this.packageName.compareTo(name);
			if (cmp == 0) {
				if (this.isForStatic == isStatic) {
					return 0;
				}
				return this.isForStatic ? -1 : 1;
			}
			return cmp;
		}
		...
		public boolean isSameGroup(PackageEntry other) {
			if (this.groupName == null) {
				...
				return this.groupName.equals(other.getGroupID()) && (this.isForStatic == other.isStatic());
			}
		}		
		...
		public boolean isComment() {
			return "!".equals(this.packageName); //$NON-NLS-1$
		}
		...
		public String toString() {
			...
			if (isComment()) {
				...
				buf.append(this.packageName); buf.append(", groupId: "); buf.append(this.groupName); buf.append("\n"); //$NON-NLS-1$ //$NON-NLS-2$
				int nImports= getNumberOfImports();
				for (int i= 0; i < nImports; i++) {
					ImportDeclEntry curr= getImportAt(i);
