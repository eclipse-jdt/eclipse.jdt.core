...
public class CompletionParser extends AssistParser {
	...
	protected static final int K_SELECTOR_INVOCATION_TYPE = COMPLETION_PARSER + 2; // whether we are inside a message
																					// send
	protected static final int K_SELECTOR_QUALIFIER = COMPLETION_PARSER + 3; // whether we are inside a message send
	protected static final int K_BETWEEN_CATCH_AND_RIGHT_PAREN = COMPLETION_PARSER + 4; // whether we are between the
																						// keyword 'catch' and the
																						// following ')'
	protected static final int K_NEXT_TYPEREF_IS_CLASS = COMPLETION_PARSER + 5; // whether the next type reference is a
																				// class
	protected static final int K_NEXT_TYPEREF_IS_INTERFACE = COMPLETION_PARSER + 6; // whether the next type reference
																					// is an interface
	protected static final int K_NEXT_TYPEREF_IS_EXCEPTION = COMPLETION_PARSER + 7; // whether the next type reference
																					// is an exception
	protected static final int K_BETWEEN_NEW_AND_LEFT_BRACKET = COMPLETION_PARSER + 8; // whether we are between the
																						// keyword 'new' and the
																						// following left braket, i.e.
																						// '[', '(' or '{'
	protected static final int K_INSIDE_THROW_STATEMENT = COMPLETION_PARSER + 9; // whether we are between the keyword
																					// 'throw' and the end of a throw
																					// statement
	protected static final int K_INSIDE_RETURN_STATEMENT = COMPLETION_PARSER + 10; // whether we are between the keyword
																					// 'return' and the end of a return
																					// statement
	protected static final int K_CAST_STATEMENT = COMPLETION_PARSER + 11; // whether we are between ')' and the end of a
																			// cast statement
	...
	private void addPotentialName(char[] potentialVariableName, int start, int end) {
		...
		if (this.potentialVariableNamesPtr >= length - 1) {
			System.arraycopy(this.potentialVariableNames, 0, this.potentialVariableNames = new char[length * 2][], 0,
					length);
			System.arraycopy(this.potentialVariableNameStarts, 0,
					this.potentialVariableNameStarts = new int[length * 2], 0, length);
			System.arraycopy(this.potentialVariableNameEnds, 0, this.potentialVariableNameEnds = new int[length * 2], 0,
					length);
		}
	...
	protected void attachOrphanCompletionNode() {
		...
		if (this.isOrphanCompletionNode) {
			...
			} else if (this.currentElement instanceof RecoveredType) { /*
																		 * if in context of a type, then persists the
																		 * identifier into a fake field return type
																		 */
					...
					if (orphan instanceof TypeReference) {
						...
						if ((length = this.expressionLengthStack[this.expressionLengthPtr]) != 0
							...
							System.arraycopy(this.expressionStack, this.expressionPtr - length + 1,
									fieldDeclaration.annotations = new Annotation[length], 0, length);
						}
			...
			if (orphan instanceof Statement) {
				/*
				 * check for completion at the beginning of method body behind an invalid signature
				 */
				...
				if (method != null) {
					...
					if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
							&& (Util.getLineNumber(orphan.sourceStart, this.scanner.lineEnds, 0,
									this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd,
			...
			if (this.expressionPtr > -1) {
				...
				if (expression == this.assistNode) {
					if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_MEMBER_VALUE_ARRAY_INITIALIZER) {
						...
						// The following if-statement is the result of inlining a call of
						// buildMoreAnnotationCompletionContext
						...
						this.currentElement.add(fakeType, 0);
						this.pendingAnnotation = fakeType;
						this.assistNodeParent = new AssistNodeParentAnnotationArrayInitializer(typeReference,
								memberValueName);
					} else if (this
							.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN) {
						if (expression instanceof SingleNameReference) {
							SingleNameReference nameReference = (SingleNameReference) expression;
							CompletionOnMemberValueName memberValueName = new CompletionOnMemberValueName(
									nameReference.token, nameReference.sourceStart, nameReference.sourceEnd);

							buildMoreAnnotationCompletionContext(memberValueName);
							return;
						} else if (expression instanceof QualifiedNameReference
								|| expression instanceof StringLiteral) {
							MemberValuePair valuePair = new MemberValuePair(VALUE, expression.sourceStart,
									expression.sourceEnd, expression);
						...
						if ((index = lastIndexOfElement(K_ATTRIBUTE_VALUE_DELIMITER)) != -1) {
							...
							if (attributeIndentifierPtr != identPtr)
								return;
							...
							MemberValuePair memberValuePair = new MemberValuePair(
									this.identifierStack[this.identifierPtr--], expression.sourceStart,
									expression.sourceEnd, expression);
			...
			if (this.currentElement instanceof RecoveredType) {
				...
				if (recoveredType.foundOpeningBrace && this.genericsPtr > -1) {
					if (this.genericsStack[this.genericsPtr] instanceof TypeParameter) {
						...
						if (detector.containsCompletionNode()) {
							this.currentElement
									.add(new CompletionOnMethodTypeParameter(new TypeParameter[] { typeParameter },
											this.compilationUnit.compilationResult()), 0);
						}
						return;
					}
				}
							...
							if (detector.containsCompletionNode()) {
								if (node == this.assistNode) {
									if (this.identifierLengthPtr > -1
											&& this.identifierLengthStack[this.identifierLengthPtr] != 0) {
					...
					if (this.assistNodeParent != null && this.assistNodeParent instanceof TypeReference) {
						if (this.currentElement instanceof RecoveredType) {
							this.currentElement = this.currentElement
									.add(new CompletionOnFieldType((TypeReference) this.assistNodeParent, false), 0);
						} else {
		...
		if (this.genericsPtr > -1) {
			...
			if (detector.containsCompletionNode()) {
				...
				if (method != null) {
					AbstractMethodDeclaration methodDecl = method.methodDeclaration;
					if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
							&& (Util.getLineNumber(node.sourceStart, this.scanner.lineEnds, 0,
									this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd,
											this.scanner.lineEnds, 0, this.scanner.linePtr))) {
						return;
					}
		...
		if (this.expressionPtr > -1) {
			...
			if (detector.containsCompletionNode()) {
				...
				if (method != null) {
					AbstractMethodDeclaration methodDecl = method.methodDeclaration;
					if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
							&& (Util.getLineNumber(expression.sourceStart, this.scanner.lineEnds, 0,
									this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd,
											this.scanner.lineEnds, 0, this.scanner.linePtr))) {
						return;
					}
				}
				if (expression == this.assistNode || (expression instanceof Assignment // https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
						&& ((Assignment) expression).expression == this.assistNode
						&& ((this.expressionPtr > 0
								&& stackHasInstanceOfExpression(this.expressionStack, this.expressionPtr - 1))
								// In case of error in compilation unit, expression stack might not have instanceof exp,
								// so try elementObjectInfoStack
								|| (this.elementPtr >= 0
										&& stackHasInstanceOfExpression(this.elementObjectInfoStack, this.elementPtr))))
						...
						|| (expression instanceof AND_AND_Expression // https://bugs.eclipse.org/bugs/show_bug.cgi?id=568934#c8
								&& (this.elementPtr >= 0 && (this.elementObjectInfoStack[this.elementPtr] == null
										|| this.elementObjectInfoStack[this.elementPtr] instanceof InstanceOfExpression)))
					...
					if (this.assistNodeParent == null && expression instanceof Assignment) {
						this.assistNodeParent = detector.getCompletionNodeParent();
					}
					return;
				} else {
		...
		if (local != null) {
			...
			if (enclosing instanceof IfStatement ifStatement) {
				if (this.currentElement instanceof RecoveredBlock recoveredBlock) {
					// RecoveredLocalVariable must be removed from its parent because the IfStatement will be added
					// instead
					...
					for (int i = 0; i < recoveredBlock.statementCount; i++) {
						...
						if (!(statement instanceof RecoveredLocalVariable recoveredLocalVariable
								&& recoveredLocalVariable.localDeclaration == local)) {
					...
					if (ifStatement.condition instanceof InstanceOfExpression iof
							&& iof.pattern instanceof TypePattern pattern) {
	...
	public Object becomeSimpleParser() {
		...
		SavedState parserState = new SavedState(this.cursorLocation, completionScanner.cursorLocation,
				this.assistNodeParent);
	...
	private void buildMoreAnnotationCompletionContext(MemberValuePair memberValuePair) {
		...
		if (memberValuePair instanceof CompletionOnMemberValueName) {
			...
			if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
				if (this.astStack[this.astPtr] instanceof MemberValuePair) {
					System.arraycopy(this.astStack, (this.astPtr -= length) + 1,
							memberValuePairs = new MemberValuePair[length - nodesToRemove], 0, length - nodesToRemove);
				}
			}
			annotation = new CompletionOnAnnotationMemberValuePair(typeReference, this.intStack[this.intPtr--],
			...
			if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
				if (this.astStack[this.astPtr] instanceof MemberValuePair) {
					System.arraycopy(this.astStack, (this.astPtr -= length) + 1,
							memberValuePairs = new MemberValuePair[length - nodesToRemove + 1], 0,
			...
			annotation = new NormalAnnotation(typeReference, this.intStack[this.intPtr--]);
	...
	private void buildMoreCompletionContext(Expression expression) {
			...
			nextElement: switch (kind) {
				case K_SELECTOR_QUALIFIER:
					...
					if (selector == THIS_CONSTRUCTOR || selector == SUPER_CONSTRUCTOR) {
						ExplicitConstructorCall call = new ExplicitConstructorCall(
								(selector == THIS_CONSTRUCTOR) ? ExplicitConstructorCall.This
						...
						if (this.expressionPtr > 0 && this.expressionLengthPtr > 0 && length == 1) {
							...
							if (this.expressionStack[this.expressionPtr - 1] != null
									&& this.expressionStack[this.expressionPtr - 1].sourceStart > start) {
						...
						if (invocType != ALLOCATION && invocType != QUALIFIED_ALLOCATION) {
							...
							switch (invocType) {
								...
								case NAME_RECEIVER:
									...
									while (this.identifierLengthPtr >= 0
											&& this.identifierLengthStack[this.identifierLengthPtr] < 0) {
										this.identifierLengthPtr--;
									}
									...
									// remove selector
									this.identifierPtr--;
									if (this.genericsPtr > -1 && this.genericsLengthPtr > -1
											&& this.genericsLengthStack[this.genericsLengthPtr] > 0) {
										...
										if (length != 0) {
											System.arraycopy(this.typeAnnotationStack,
													(this.typeAnnotationPtr -= length) + 1,
													typeAnnotations = new Annotation[length], 0, length);
											problemReporter().misplacedTypeAnnotations(typeAnnotations[0],
													typeAnnotations[typeAnnotations.length - 1]);
										}
									}
									// consume the receiver
									int identifierLength = this.identifierLengthStack[this.identifierLengthPtr];
									if (this.identifierPtr > -1 && identifierLength > 0
											&& this.identifierPtr + 1 >= identifierLength) {
				...
				case K_INSIDE_RETURN_STATEMENT:
					if (info == this.bracketDepth) {
						ReturnStatement returnStatement = new ReturnStatement(expression, expression.sourceStart,
				...
				case K_UNARY_OPERATOR:
					if (this.expressionPtr > -1) {
						...
						switch (info) {
							case PLUS_PLUS:
								operatorExpression = new PrefixExpression(expression, IntLiteral.One, PLUS,
										expression.sourceStart);
								break;
							case MINUS_MINUS:
								operatorExpression = new PrefixExpression(expression, IntLiteral.One, MINUS,
										expression.sourceStart);
								break;
							default:
				...
				case K_ARRAY_INITIALIZER:
					...
					if (this.expressionLengthPtr > -1 && this.expressionPtr > -1
					...
					if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_ARRAY_CREATION) {
						...
						allocationExpression.type.bits |= ASTNode.IgnoreRawTypeCheck; // no need to worry about raw type
																						// usage
						int length = this.expressionLengthStack[this.expressionLengthPtr];
				...
				case K_ASSISGNMENT_OPERATOR:
					if (this.expressionPtr > 0 && this.expressionStack[this.expressionPtr - 1] != null) {
						...
						if (info == EQUAL) {
							assignment = new Assignment(this.expressionStack[this.expressionPtr - 1], expression,
									expression.sourceEnd);
						} else {
							assignment = new CompoundAssignment(this.expressionStack[this.expressionPtr - 1],
									expression, info, expression.sourceEnd);
						}
				...
				case K_BETWEEN_LEFT_AND_RIGHT_BRACKET:
					...
					if (this.identifierPtr < 0 && this.expressionPtr > 0
							&& this.expressionStack[this.expressionPtr] == expression) {
						arrayReference = new ArrayReference(this.expressionStack[this.expressionPtr - 1], expression);
					} else {
						arrayReference = new ArrayReference(getUnspecifiedReferenceOptimized(), expression);
					}
				...
				case K_BETWEEN_CASE_AND_COLON:
					if (this.expressionPtr > 0) {
						...
						CaseStatement caseStatement = new CaseStatement(new Expression[] { expression },
				...
				case K_BETWEEN_IF_AND_RIGHT_PAREN:
					IfStatement ifStatement = new IfStatement(expression,
							new EmptyStatement(expression.sourceEnd, expression.sourceEnd), expression.sourceStart,
							expression.sourceEnd);
					this.assistNodeParent = ifStatement;
					break nextElement;
				case K_BETWEEN_WHILE_AND_RIGHT_PAREN:
					WhileStatement whileStatement = new WhileStatement(expression,
							new EmptyStatement(expression.sourceEnd, expression.sourceEnd), expression.sourceStart,
							expression.sourceEnd);
				...
				case K_INSIDE_FOR_CONDITIONAL: // https://bugs.eclipse.org/bugs/show_bug.cgi?id=253008
					ForStatement forStatement = new ForStatement(new Statement[0], expression, new Statement[0],
							new EmptyStatement(expression.sourceEnd, expression.sourceEnd), false,
							expression.sourceStart, expression.sourceEnd);
				...
				case K_BETWEEN_SYNCHRONIZED_AND_RIGHT_PAREN:
					SynchronizedStatement synchronizedStatement = new SynchronizedStatement(expression, new Block(0),
							expression.sourceStart, expression.sourceEnd);
					this.assistNodeParent = synchronizedStatement;
					break nextElement;
				case K_INSIDE_THROW_STATEMENT:
					if (info == this.bracketDepth) {
						ThrowStatement throwStatement = new ThrowStatement(expression, expression.sourceStart,
								expression.sourceEnd);
				...
				case K_INSIDE_ASSERT_EXCEPTION:
					if (info == this.bracketDepth) {
						AssertStatement assertStatement = new AssertStatement(expression,
								new TrueLiteral(expression.sourceStart, expression.sourceStart),
								expression.sourceStart);
						this.assistNodeParent = assertStatement;
					}
					break nextElement;
		...
		if (this.assistNodeParent != null) {
			this.currentElement = this.currentElement
					.add(buildMoreCompletionEnclosingContext((Statement) this.assistNodeParent), 0);
		} else {
			if (this.currentElement instanceof RecoveredField && !(this.currentElement instanceof RecoveredInitializer)
				...
				if (lastIndexOfElement(K_LAMBDA_EXPRESSION_DELIMITER) <= lastIndexOfElement(
	...
	private boolean assembleSwitch(Expression innerStatement) {
		if (lastIndexOfElement(K_SWITCH_LABEL) > -1 && this.expressionPtr > -1 && this.astPtr > -1
			...
			if (caseStatement != null && popBlockContaining(caseStatement)) {
				...
				if (length != 0 && caseStatement.sourceStart > switchStatement.expression.sourceEnd) {
					...
					System.arraycopy(this.astStack, casePos + 1, switchStatement.statements, 1, length);
	...
	private Statement buildMoreCompletionEnclosingContext(Statement statement) {
		...
		int index = -1;
		/*
		 * What happens here? When we have an "instanceof" after the last K_CONTROL_STATEMENT_DELIMITER (which
		 * represents if or else-if), the former is taken to be the current point. Otherwise, the standard rule applies:
		 * i.e. pick the block if it comes after the K_CONTROL_STATEMENT_DELIMITER, otherwise pick the
		 * K_BLOCK_DELIMITER.
		 */
		...
		while (index >= 0) {
			// Try to find an enclosing if statement with instanceof even if one is not found immediately preceding the
			// completion node.
			// A if statement is only chosen if it has a instanceof or the completion node is in the if statment it
			// self.
			...
			if ((kind == K_BLOCK_DELIMITER || kind == K_CONTROL_STATEMENT_DELIMITER
					|| kind == K_BETWEEN_INSTANCEOF_AND_RPAREN) // same set as above
				...
				ifStatement = new IfStatement(condition,
						statement == condition ? new EmptyStatement(condition.sourceEnd, condition.sourceEnd)
								: statement,
						condition.sourceStart, statement.sourceEnd);
				index--;
		...
		while (index >= 0) {
			if (this.elementInfoStack[index] == IF && isInstanceOfGuard(this.elementObjectInfoStack[index])) {
				...
				ifStatement = new IfStatement(condition, ifStatement, condition.sourceStart, ifStatement.sourceEnd);
				this.elementPtr = index;
			}
			index--;
		}
						...
						if (this.identifierLengthPtr > -1
							...
							if (this.currentElement instanceof RecoveredType) {
								...
								if (prevKind == K_BETWEEN_NEW_AND_LEFT_BRACKET) {
									...
									if (isInsideReturn()) {
										ReturnStatement returnStatement = new ReturnStatement(exp, exp.sourceStart,
												exp.sourceEnd);
	...
	private void buildMoreTryStatementCompletionContext(TypeReference exceptionRef) {
		if (this.astLengthPtr > 0 && this.astPtr > 2 && this.astStack[this.astPtr - 1] instanceof Block
		...
		} else if (this.astLengthPtr > -1 && this.astPtr > 0 && this.astStack[this.astPtr - 1] instanceof Block) {
			TryStatement tryStatement = new TryStatement();
	...
	private boolean checkClassInstanceCreation() {
		if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BETWEEN_NEW_AND_LEFT_BRACKET) {
			...
			if (this.invocationType == ALLOCATION) {
				...
				allocExpr.sourceEnd = type.sourceEnd;
				this.expressionStack[this.qualifier] = allocExpr; // attach it now (it replaces the qualifier
																	// expression)
				this.isOrphanCompletionNode = false;
			}
	...
	/**
	 * Checks if the completion is on the dot following an array type, a primitive type or an primitive array type.
	 * Returns whether we found a completion node.
	 */
	private boolean checkClassLiteralAccess() {
		if (this.identifierLengthPtr >= 1 && this.previousToken == TokenNameDOT) { // (NB: the top id length is 1 and it
																					// is for the completion identifier)
			int length;
			...
			if ((length = this.identifierLengthStack[this.identifierLengthPtr - 1]) < 0) {
				...
				// typeRef.sourceEnd = typeRef.sourceStart + typeRef.token.length; // NB: It's ok to use the length of
				// the token since it doesn't contain any unicode
	...
	private boolean checkKeywordAndRestrictedIdentifiers() {
		if (this.currentElement instanceof RecoveredUnit) {
			...
			if ((index = this.indexOfAssistIdentifier()) > -1) {
				...
				if (unit.typeCount == 0 && unit.importCount == 0 && this.lastModifiers == ClassFileConstants.AccDefault
				...
				if (count != 0) {
					System.arraycopy(keywordsAndRestrictedIndentifiers, 0,
							keywordsAndRestrictedIndentifiers = new char[count][], 0, count);
	...
	private enum ModuleKeyword {
		FIRST_ALL, TO, PROVIDES_WITH, NOT_A_KEYWORD
	}

	private ModuleKeyword getKeyword() {
		ModuleKeyword keyword = ModuleKeyword.FIRST_ALL;
		if (isInModuleStatements()) {
			if (foundToken(K_AFTER_PACKAGE_IN_PACKAGE_VISIBILITY_STATEMENT))
				keyword = ModuleKeyword.TO;
			else if (foundToken(K_AFTER_NAME_IN_PROVIDES_STATEMENT))
				keyword = ModuleKeyword.PROVIDES_WITH;
			else
				keyword = ModuleKeyword.NOT_A_KEYWORD;
		}
		return keyword;
	}

	private char[][] getModuleKeywords(ModuleKeyword keyword) {
		if (keyword == ModuleKeyword.TO)
			return new char[][] { Keywords.TO };
		else if (keyword == ModuleKeyword.PROVIDES_WITH)
			return new char[][] { Keywords.WITH };
		else
			return new char[][] { Keywords.EXPORTS, Keywords.OPENS, Keywords.REQUIRES, Keywords.PROVIDES,
					Keywords.USES };
	}
	...
	private boolean checkModuleInfoConstructs() {
		...
		if (this.currentElement instanceof RecoveredModule) {
			...
			if (checkModuleInfoKeyword(module, index))
				return true;
		} else {
	...
	private boolean checkInstanceofKeyword() {
		if (isInsideMethod()) {
			int kind = topKnownElementKind(COMPLETION_OR_ASSIST_PARSER);
			int index;
			if (kind != K_BLOCK_DELIMITER && (index = indexOfAssistIdentifier()) > -1 && this.expressionPtr > -1
					&& this.expressionLengthStack[this.expressionPtr] == 1) {

				int ptr = this.identifierPtr - this.identifierLengthStack[this.identifierLengthPtr] + index + 1;
				if (this.identifierStack[ptr].length > 0
						&& CharOperation.prefixEquals(this.identifierStack[ptr], Keywords.INSTANCEOF)) {
					this.assistNode = new CompletionOnKeyword3(this.identifierStack[ptr],
							this.identifierPositionStack[ptr], Keywords.INSTANCEOF);
					this.lastCheckPoint = this.assistNode.sourceEnd + 1;
					this.isOrphanCompletionNode = true;
					return true;
				}
	...
	/**
	 * Checks if the completion is inside a method invocation or a constructor invocation. Returns whether we found a
	 * completion node.
	 */
	private boolean checkInvocation() {
		Expression topExpression = this.expressionPtr >= 0 ? this.expressionStack[this.expressionPtr] : null;
		...
		if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER
				&& ((isEmptyNameCompletion = topExpression == this.assistNode && isEmptyNameCompletion()) // e.g. it is
																											// something
																											// like
																											// "this.fred([cursor]"
																											// but it is
																											...
																											// +
																											// [cursor]"
						|| (isEmptyAssistIdentifier = this.indexOfAssistIdentifier() >= 0
								&& this.identifierStack[this.identifierPtr].length == 0))) { // e.g. it is something
																								// like "this.fred(1
																								// [cursor]"
			...
			if (invocType != ALLOCATION && invocType != QUALIFIED_ALLOCATION) {
				...
				messageSend.sourceStart = (int) (this.identifierPositionStack[selectorPtr] >> 32); // this.cursorLocation
																									// + 1;
				...
				if (selectorPtr == THIS_CONSTRUCTOR || selectorPtr == SUPER_CONSTRUCTOR) {
					...
					CompletionOnExplicitConstructorCall call = new CompletionOnExplicitConstructorCall(
							(selectorPtr == THIS_CONSTRUCTOR) ? ExplicitConstructorCall.This
									: ExplicitConstructorCall.Super);
	...
	private boolean checkLabelStatement() {
		if (isInsideMethod() || isInsideFieldInitialization()) {
			...
			if (kind != K_INSIDE_BREAK_STATEMENT && kind != K_INSIDE_CONTINUE_STATEMENT)
				return false;
			...
			this.assistNode = kind == K_INSIDE_BREAK_STATEMENT
					? new CompletionOnBreakStatement(this.identifierStack[this.identifierPtr--],
							(int) (position >>> 32), (int) position, labels)
					: new CompletionOnContinueStatement(this.identifierStack[this.identifierPtr--],
							(int) (position >>> 32), (int) position, labels);
			this.lastCheckPoint = this.assistNode.sourceEnd + 1;
			this.isOrphanCompletionNode = true;
			return true;
	...
	/**
	 * Checks if the completion is on a member access (i.e. in an identifier following a dot). Returns whether we found
	 * a completion node.
	 */
	...
	/**
	 * Checks if the completion is on a method call (returning a type). Returns whether we found
	 * a completion node.
	 */
	private boolean checkMethodResult() {
		if (this.previousToken == TokenNameRPAREN && this.expressionPtr >= 0) {
			if (this.expressionStack[this.expressionPtr] instanceof MessageSend
					|| this.expressionStack[this.expressionPtr] instanceof AllocationExpression) {
				// the receiver is an expression
				pushCompletionOnMethodCallResultOnExpressionStack();
				return true;
			}
		}
		return false;
	...
	/**
	 * Checks if the completion is on a name reference. Returns whether we found a completion node.
	 */
	private boolean checkNameCompletion() {
		/*
		 * We didn't find any other completion, but the completion identifier is on the identifier stack, so it can only
		 * be a completion on name. Note that we allow the completion on a name even if nothing is expected (e.g. foo()
		 * b[cursor] would be a completion on 'b'). This policy gives more to the user than he/she would expect, but
		 * this simplifies the problem. To fix this, the recovery must be changed to work at a 'statement' granularity
	...
	private boolean checkParemeterizedMethodName() {
		if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_PARAMETERIZED_METHOD_INVOCATION
			...
			if (this.identifierLengthPtr > -1 && this.genericsLengthPtr > -1
					...
					case NO_RECEIVER: // this case occurs with 'bar().foo'
						if (this.expressionPtr > -1 && this.expressionLengthStack[this.expressionLengthPtr] == 1) {
							...
							System.arraycopy(this.genericsStack, this.genericsPtr + 1,
									m.typeArguments = new TypeReference[length], 0, length);
							this.intPtr--;
	...
	/**
	 * Checks if the completion is in the context of a method and on the type of one of its arguments Returns whether we
	 * found a completion node.
	 */
	private boolean checkRecoveredMethod() {
		if (this.currentElement instanceof RecoveredMethod) {
			...
			/*
			 * check if on line with an error already - to avoid completing inside illegal type names e.g. int[<cursor>
			 */
			if (this.lastErrorEndPosition <= this.cursorLocation && Util.getLineNumber(this.lastErrorEndPosition,
					this.scanner.lineEnds, 0, this.scanner.linePtr) == Util.getLineNumber(
							((CompletionScanner) this.scanner).completedIdentifierStart, this.scanner.lineEnds, 0,
							this.scanner.linePtr)) {
				return false;
			}
			RecoveredMethod recoveredMethod = (RecoveredMethod) this.currentElement;
			/* only consider if inside method header */
			if (!recoveredMethod.foundOpeningBrace && this.lastIgnoredToken == TokenNameInvalid) {
	...
	private boolean checkMemberValueName() {
		...
		if (this.identifierPtr > -1 && this.identifierLengthPtr > -1
	...
	/**
	 * Checks if the completion is in the context of a type and on a type reference in this type. Persists the
	 * identifier into a fake field return type Returns whether we found a completion node.
	 */
	private boolean checkRecoveredType() {
		if (this.currentElement instanceof RecoveredType) {
			...
			if (this.lastErrorEndPosition <= this.cursorLocation
					...
					&& Util.getLineNumber(this.lastErrorEndPosition, this.scanner.lineEnds, 0,
							this.scanner.linePtr) == Util.getLineNumber(
									((CompletionScanner) this.scanner).completedIdentifierStart, this.scanner.lineEnds,
	...
	private void classHeaderExtendsOrImplements(boolean isInterface, boolean isRecord, boolean isEnum) {
		if (this.currentElement != null && this.currentToken == TokenNameIdentifier
	...
	/*
	 * Check whether about to shift beyond the completion token. If so, depending on the context, a special node might
	 * need to be created and attached to the existing recovered structure so as to be remember in the resulting parsed
	 * structure.
	 */
	public void completionIdentifierCheck() {
		// if (assistNode != null) return;

		if (checkMemberValueName())
			return;

		if (checkKeywordAndRestrictedIdentifiers())
			return;
		if (checkModuleInfoConstructs())
			return;
		...
		// if not in a method in non diet mode and if not inside a field initializer, only record references attached to
		// types
		if (!(isInsideMethod() && !this.diet) && !isIndirectlyInsideFieldInitialization()
				&& !isIndirectlyInsideEnumConstantnitialization() && !isInsideAttributeValue() && !isInsideModuleInfo())
			return;

		/*
		 * In some cases, the completion identifier may not have yet been consumed, e.g. int.[cursor] This is because
		 * the grammar does not allow any (empty) identifier to follow a base type. We thus have to manually force the
		 * identifier to be consumed (that is, pushed).
		 */
		if (assistIdentifier() == null && this.currentToken == TokenNameIdentifier) { // Test below copied from
																						// CompletionScanner.getCurrentIdentifierSource()
			if (this.cursorLocation < this.scanner.startPosition
					&& this.scanner.currentPosition == this.scanner.startPosition) { // fake empty identifier got issued
				this.pushIdentifier();
			} else if (this.cursorLocation + 1 >= this.scanner.startPosition
					&& this.cursorLocation < this.scanner.currentPosition) {
				this.pushIdentifier();
			}
		}
		...
		if (checkClassLiteralAccess())
			return;
		if (checkInstanceofKeyword())
			return;

		// if the completion was not on an empty name, it can still be inside an invocation (e.g.
		// this.fred("abc"[cursor])
		// (NB: Put this check before checkNameCompletion() because the selector of the invocation can be on the
		// identifier stack)
		if (checkInvocation())
			return;

		if (checkParemeterizedType())
			return;
		if (checkParemeterizedMethodName())
			return;
		if (checkLabelStatement())
			return;
	...
	protected void consumeCastExpressionWithPrimitiveType() {
		...
		this.expressionStack[this.expressionPtr] = cast = new CastExpression(
				...
				castType = (TypeReference) this.expressionStack[this.expressionPtr]);
	...
	protected void consumeCatchFormalParameter() {
		if (this.indexOfAssistIdentifier() < 0) {
			...
			this.intPtr -= 2;
			CompletionOnArgumentName arg = new CompletionOnArgumentName(identifierName, namePositions, type,
			...
			/*
			 * if incomplete method header, listLength counter will not have been reset, indicating that some arguments
	...
	protected void consumeClassDeclaration() {
		if (this.astPtr >= 0) {
			...
			checkComment();
			if (this.javadoc != null && this.cursorLocation > this.javadoc.sourceStart
					&& this.cursorLocation < this.javadoc.sourceEnd) {
				// completion is in an orphan javadoc comment => replace in last read declaration to allow completion
				// resolution
	...
	protected void consumeEmptyStatement() {
		...
		if (this.shouldStackAssistNode && nodeToAttach != null) {
			for (int ptr = this.astPtr; ptr >= 0; ptr--) {
				if (new CompletionNodeDetector(nodeToAttach, this.astStack[ptr]).containsCompletionNode()) {
					...
					this.astLengthStack[++this.astLengthPtr] = 0; // start an empty list instead of the unnecessary
																	// empty statement
		...
		/*
		 * Sneak in the assist node. The reason we can't do that when we see the assist node is that we don't know
		 * whether it is the first or subsequent statement in a block to be able to decide whether to call
		 * contactNodeLists. See Parser.consumeBlockStatement(s)
		 */
	...
	protected void consumeBlockStatement() {
		...
		if (this.currentElement != null && this.scanner.currentPosition > this.cursorLocation
			...
			// if the new statement is the 1st child of a not-yet materialized control statement, synthesize that
			// control statement (if) now:
			Statement stmt = (Statement) this.astStack[this.astPtr];
	...
	protected void consumeEnterVariable() {
		...
		if (isLocalDeclaration || indexOfAssistIdentifier() < 0 || variableIndex != 0) {
			...
			if (this.pendingAnnotation != null && this.assistNode != null && this.currentElement != null
					&& this.currentElement instanceof RecoveredMethod && !this.currentElement.foundOpeningBrace
			...
			// recovery
			if (this.currentElement != null) {
				if (!checkKeywordAndRestrictedIdentifiers() && !(this.currentElement instanceof RecoveredUnit
						&& ((RecoveredUnit) this.currentElement).typeCount == 0)) {
					...
					if (!(this.currentElement instanceof RecoveredType) && (this.currentToken == TokenNameDOT || (Util
							.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util
									.getLineNumber(nameSourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr)))) {
					...
					if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
						System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1,
								completionFieldDecl.annotations = new Annotation[length], 0, length);
					}
	...
	protected void consumeExitVariableWithInitialization() {
		...
		// do not keep the initialization if completion is not inside, except for var typed local where initializer must
		// be preserved for LVTI
	...
	/*
	 * Copy of code from superclass with the following change: If the cursor location is on the field access, then
	 * create a CompletionOnMemberAccess instead.
	 */
	@Override
	...
	protected void consumeSingleVariableDeclarator(boolean isVarArgs) {
		...
		if (this.indexOfAssistIdentifier() < 0 || this.parsingRecordComponents) {
			...
			Annotation[][] annotationsOnExtendedDimensions = extendedDimensions == 0 ? null
					: getAnnotationsOnDimensions(extendedDimensions);
			...
			if (isVarArgs) {
				...
				if ((length = this.typeAnnotationLengthStack[this.typeAnnotationLengthPtr--]) != 0) {
					System.arraycopy(this.typeAnnotationStack, (this.typeAnnotationPtr -= length) + 1,
							varArgsAnnotations = new Annotation[length], 0, length);
				}
			}
			...
			if (isVarArgs || extendedDimensions != 0) {
				if (isVarArgs) {
					type = augmentTypeWithAdditionalDimensions(type, 1,
							varArgsAnnotations != null ? new Annotation[][] { varArgsAnnotations } : null, true);
				}
				if (extendedDimensions != 0) { // combination illegal.
					type = augmentTypeWithAdditionalDimensions(type, extendedDimensions,
							annotationsOnExtendedDimensions, false);
				}
		...
		if (this.indexOfAssistIdentifier() < 0) {
			...
			if (this.indexOfAssistIdentifier() != 0
				...
				if (this.currentElement != null) {
					...
					if (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util
							.getLineNumber((int) (selectorSource >>> 32), this.scanner.lineEnds, 0,
									this.scanner.linePtr)) {
						FieldDeclaration completionFieldDecl = new CompletionOnFieldType(type, false);
						// consume annotations
						int length;
						...
						CompletionOnMethodReturnType md = new CompletionOnMethodReturnType(type,
					...
					if (!recoveredMethod.foundOpeningBrace) {
						...
						if (method.thrownExceptions == null) {
							CompletionOnKeyword1 completionOnKeyword = new CompletionOnKeyword1(
									this.identifierStack[ptr], this.identifierPositionStack[ptr], Keywords.THROWS);
		...
		if (this.currentElement != null && this.currentToken == TokenNameIdentifier
			...
			if ((index = this.indexOfAssistIdentifier()) > -1) {
				...
				if (!recoveredMethod.foundOpeningBrace) {
					...
					if (method.thrownExceptions == null) {
						CompletionOnKeyword1 completionOnKeyword = new CompletionOnKeyword1(this.identifierStack[ptr],
								this.identifierPositionStack[ptr], Keywords.THROWS);
						method.thrownExceptions = new TypeReference[] { completionOnKeyword };
						recoveredMethod.foundOpeningBrace = true;
						this.assistNode = completionOnKeyword;
						this.lastCheckPoint = completionOnKeyword.sourceEnd + 1;
	...
	protected void consumeAnnotationName() {
		...
		/*
		 * retrieve identifiers subset and whole positions, the assist node positions should include the entire replaced
		 * source.
		 */
		...
		if (index == 0) {
			...
			typeReference = createSingleAssistTypeReference(assistIdentifier(), positions[0]);
		} else {
			/* assist inside subsequent identifier */
			typeReference = createQualifiedAssistTypeReference(subset, assistIdentifier(), positions);
		}
	...
	protected void consumeLambdaExpression() {
		...
		if (this.assistNode == null || !(this.assistNode.sourceStart >= expression.sourceStart
				&& this.assistNode.sourceEnd <= expression.sourceEnd))
	...
	protected void consumeMemberValueAsName() {
		if ((indexOfAssistIdentifier()) < 0) {
			...
			if (topKnownElementKind == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN
					|| topKnownElementKind == K_MEMBER_VALUE_ARRAY_INITIALIZER) {
				this.restartRecovery = true;
			}
		}
	}
	...
	protected void consumeNormalAnnotation(boolean isTypeAnnotation) {
		if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN
			...
			popElement(K_BETWEEN_ANNOTATION_NAME_AND_RPAREN);
			if (this.expressionPtr >= 0
					&& this.expressionStack[this.expressionPtr] instanceof CompletionOnMarkerAnnotationName) {
				...
				if (this.currentElement != null) {
					...
					if (this.currentElement instanceof RecoveredAnnotation) {
						this.currentElement = ((RecoveredAnnotation) this.currentElement).addAnnotation(annotation,
								this.identifierPtr);
					}
				}
	...
	protected TerminalToken fetchNextToken() throws InvalidInputException {
		...
		if (token != TokenNameEOF && this.scanner.currentPosition > this.cursorLocation) {
			if (!this.diet || this.dietInt != 0) { // do this also when parsing field initializers:
				if (this.currentToken == TokenNameIdentifier && this.identifierStack[this.identifierPtr].length == 0) {
					if (Scanner.isLiteral(token)) {
						// <emptyAssistIdentifier> <someLiteral> is illegal and most likely the literal should be
						// replaced => discard it now
						return fetchNextToken();
					}
					if (token == TokenNameIdentifier) {
						// empty completion identifier followed by another identifier likely means the 2nd id should
						// start another parse node.
						// To cleanly separate them find a suitable separator:
						this.scanner.currentPosition = this.scanner.startPosition; // return to startPosition after this
																					// charade
			...
			if (!this.diet) { // only when parsing a method body:
				if (this.expressionPtr <= -1 && !requireExtendedRecovery()) {
					this.scanner.eofPosition = this.cursorLocation + 1; // revert to old strategy where we stop parsing
																		// right at the cursor

					// stop immediately or deferred?
					if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BLOCK_DELIMITER // -> not within a complex
																								// structure?
							&& this.scanner.startPosition > this.cursorLocation + 1 // -> is current token entirely
																					// beyond cursorLocation?
	...
	/*
	 * Variant of parse() without side effects that stops when another token would need to be fetched. Returns the name
	 * of the last reduced rule, or empty string if nothing reduced, or null if error was detected.
	 */
	String reduce(TerminalToken token) {
		...
		for (;;) {
			...
			if (++myStackTop >= stackLength) {
				System.arraycopy(myStack, 0, myStack = new int[stackLength + StackIncrement], 0, stackLength);
			}
	...
	protected void consumeToken(TerminalToken token) {
		...
		if (previous == TokenNameIdentifier && token == TokenNameAT308 && this.currentElement == null
			...
			this.scanner.eofPosition = this.cursorLocation < Integer.MAX_VALUE ? this.cursorLocation + 1
					: this.cursorLocation;
		}
		...
		// in method bodies)
		if (token == TokenNameIdentifier && this.identifierStack[this.identifierPtr] == assistIdentifier()
				&& this.currentElement == null && (!isIndirectlyInsideLambdaExpression())
		...
		if (isInsideMethod() || isInsideFieldInitialization() || isInsideAttributeValue()
			...
			switch (token) {
				case TokenNameDOT:
					switch (previous) {
						...
						case TokenNameIdentifier: // e.g. bar[.]fred()
							if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) != K_BETWEEN_NEW_AND_LEFT_BRACKET) {
								if (this.identifierPtr != prevIdentifierPtr) { // if identifier has been consumed, e.g.
																				// this.x[.]fred()
									this.invocationType = EXPLICIT_RECEIVER;
								} else {
				...
				case TokenNamenew:
					...
					this.qualifier = this.expressionPtr; // NB: even if there is no qualification, set it to the
															// expression ptr so that the number of arguments are
															// correctly computed
				...
				case TokenNameLPAREN:
					if (this.invocationType == NO_RECEIVER || this.invocationType == NAME_RECEIVER
							|| this.invocationType == SUPER_RECEIVER) {
						this.qualifier = this.expressionPtr; // remenber the last expression so that arguments are
																// correctly computed
					}
					switch (previous) {
						case TokenNameUNDERSCORE:
						case TokenNameIdentifier: // e.g. fred[(]) or foo.fred[(])
							if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
								...
								if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER,
										1) == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN
										&& (info = topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER, 1)
												& LPAREN_NOT_CONSUMED) != 0) {
									popElement(K_SELECTOR);
									popElement(K_BETWEEN_ANNOTATION_NAME_AND_RPAREN);
									if ((info & ANNOTATION_NAME_COMPLETION) != 0) {
										this.pushOnElementStack(K_BETWEEN_ANNOTATION_NAME_AND_RPAREN,
												LPAREN_CONSUMED | ANNOTATION_NAME_COMPLETION);
									} else {
						...
						case TokenNamethis: // explicit constructor invocation, e.g. this[(]1, 2)
							if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
								this.pushOnElementStack(K_SELECTOR_INVOCATION_TYPE,
										(this.invocationType == QUALIFIED_ALLOCATION) ? QUALIFIED_ALLOCATION
												: ALLOCATION);
								this.pushOnElementStack(K_SELECTOR_QUALIFIER, this.qualifier);
							}
							this.qualifier = -1;
						...
						case TokenNameUNSIGNED_RIGHT_SHIFT: // or Fred<X<X<X>>>[(]1, 2)
							if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
								...
								if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BINARY_OPERATOR
										&& ((info = topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER, 1)) == GREATER
												|| info == RIGHT_SHIFT || info == UNSIGNED_RIGHT_SHIFT)) {
				...
				case TokenNameLBRACE:
					int kind = topKnownElementKind(COMPLETION_OR_ASSIST_PARSER);
					if (kind == K_FIELD_INITIALIZER_DELIMITER || kind == K_LOCAL_INITIALIZER_DELIMITER
					...
					switch (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
						...
						case K_BETWEEN_IF_AND_RIGHT_PAREN:
							if (topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER) == this.bracketDepth) {
								...
								pushOnElementStack(K_CONTROL_STATEMENT_DELIMITER, IF,
										this.expressionStack[this.expressionPtr]);
							}
							break;
						case K_BETWEEN_WHILE_AND_RIGHT_PAREN:
	...
	@Override
	protected void consumeInvocationExpression() { // on error, a message send's error reductions will take the
													// expression path rather than the statement path since that is a
													// dead end.
	...
	protected void consumeTypeParameterHeader() {
		...
		TypeParameter typeParameter = (TypeParameter) this.genericsStack[this.genericsPtr];
		if (typeParameter.type != null || (typeParameter.bounds != null && typeParameter.bounds.length > 0))
			return;
		...
		CompletionOnKeyword1 keyword = new CompletionOnKeyword1(this.identifierStack[this.identifierPtr],
				this.identifierPositionStack[this.identifierPtr], Keywords.EXTENDS);
	...
	protected void consumeWildcard() {
		...
		CompletionOnKeyword1 keyword = new CompletionOnKeyword1(this.identifierStack[this.identifierPtr],
				this.identifierPositionStack[this.identifierPtr], new char[][] { Keywords.EXTENDS, Keywords.SUPER });
	...
	@Override
	public NameReference createQualifiedAssistNameReference(char[][] previousIdentifiers, char[] assistName,
			long[] positions) {
		return new CompletionOnQualifiedNameReference(previousIdentifiers, assistName, positions,
	...
	private TypeReference checkAndCreateModuleQualifiedAssistTypeReference(char[][] previousIdentifiers,
		...
		if (isInUsesStatement())
			return new CompletionOnUsesQualifiedTypeReference(previousIdentifiers, assistName, positions);
		if (isInProvidesStatement()) {
			if (isAfterWithClause())
				return new CompletionOnProvidesImplementationsQualifiedTypeReference(previousIdentifiers, assistName,
						positions);
	...
	@Override
	public TypeReference createQualifiedAssistTypeReference(char[][] previousIdentifiers, char[] assistName,
			long[] positions) {
		switch (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
			...
			default:
				TypeReference ref = checkAndCreateModuleQualifiedAssistTypeReference(previousIdentifiers, assistName,
						positions);
				if (ref != null)
					return ref;
				return new CompletionOnQualifiedTypeReference(previousIdentifiers, assistName, positions,
	...
	public TypeReference createParameterizedQualifiedAssistTypeReference(char[][] previousIdentifiers,
			TypeReference[][] typeArguments, char[] assistName, TypeReference[] assistTypeArguments, long[] positions) {
		...
		if (!isParameterized) {
			...
			switch (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
				case K_NEXT_TYPEREF_IS_EXCEPTION:
					if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BETWEEN_CATCH_AND_RIGHT_PAREN)
						this.isOrphanCompletionNode = true;
					return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments,
							assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_EXCEPTION);
				case K_NEXT_TYPEREF_IS_CLASS:
					return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments,
							assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_CLASS);
				case K_NEXT_TYPEREF_IS_INTERFACE:
					return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments,
							assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_INTERFACE);
				default:
					return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments,
							assistName, positions);
			}
		}
	}
		...
		if (!isInsideMethod()) {
			if (isInsideFieldInitialization()) {
				return new CompletionOnSingleNameReference(assistName, position,
						new char[][] { Keywords.FALSE, Keywords.TRUE }, false, isInsideAttributeValue());
			}
			...
			if ((kind == K_BLOCK_DELIMITER || kind == K_LAMBDA_EXPRESSION_DELIMITER)
					&& this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == DO) {
				return new CompletionOnKeyword3(assistName, position, Keywords.WHILE);
					...
					&& this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == TRY) {
				return new CompletionOnKeyword3(assistName, position, new char[][] { Keywords.CATCH, Keywords.FINALLY },
						true);
			} else if (kind == K_BLOCK_DELIMITER && topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER) == SWITCH) {
	...
	private TypeReference checkAndCreateModuleSingleAssistTypeReference(char[] assistName, long position) {
		if (isInUsesStatement())
			return new CompletionOnUsesSingleTypeReference(assistName, position);
		if (isInProvidesStatement()) {
			if (isAfterWithClause())
				return new CompletionOnProvidesImplementationsSingleTypeReference(assistName, position);
		...
		return new CompletionOnSingleTypeReference(assistName, position, keywords.toArray(char[][]::new),
				canBeCtorCall);
	}
		...
		switch (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
			case K_NEXT_TYPEREF_IS_EXCEPTION:
				...
				return new CompletionOnSingleTypeReference(assistName, position,
						CompletionOnSingleTypeReference.K_EXCEPTION);
			case K_NEXT_TYPEREF_IS_CLASS:
				return new CompletionOnSingleTypeReference(assistName, position,
						CompletionOnSingleTypeReference.K_CLASS);
			case K_NEXT_TYPEREF_IS_INTERFACE:
				return new CompletionOnSingleTypeReference(assistName, position,
						CompletionOnSingleTypeReference.K_INTERFACE);
			default:
	...
	@Override
	public TypeReference createParameterizedSingleAssistTypeReference(TypeReference[] typeArguments, char[] assistName,
			long position) {
	...
	protected StringLiteral createStringLiteral(char[] token, int start, int end, int lineNumber) {
		if (start <= this.cursorLocation && this.cursorLocation <= end) {
			...
			if (this.cursorLocation != end || end == contentEnd) {
				CompletionOnStringLiteral stringLiteral = new CompletionOnStringLiteral(token, start, end, contentStart,
	...
	protected TypeReference augmentTypeWithAdditionalDimensions(TypeReference typeRef, int additionalDimensions,
		...
		TypeReference result = super.augmentTypeWithAdditionalDimensions(typeRef, additionalDimensions,
				additionalAnnotations, isVarargs);
		...
		if ((length = this.identifierLengthStack[this.identifierLengthPtr--]) == 1) {
			...
			if (this.assistNode == null && start <= this.cursorLocation && end >= this.cursorLocation) {
				...
				if (previousCount > 0) {
					...
					ref = new CompletionOnQualifiedNameReference(subset, tokens[previousCount], positions,
							isInsideAttributeValue());
				} else {
					// with only one token up-to cursorLocation avoid a bogus qualifiedNameReference (simply skipping
					// the remainder):
	...
	private boolean isAlreadyPotentialName(int identifierStart) {
		if (this.potentialVariableNamesPtr < 0)
			return false;
	...
	protected int indexOfAssistIdentifier(boolean useGenericsStack) {
		if (this.record)
			return -1; // when names are recorded there is no assist identifier
	...
	/**
	 * Returns whether the completion is just after an array type e.g. String[].[cursor]
	 */
	...
	private boolean isEmptyNameCompletion() {
		return this.assistNode != null && this.assistNode instanceof CompletionOnSingleNameReference
	...
	private MessageSend internalNewMessageSend() {
		...
		if (this.assistNode == null && this.lParenPos > this.cursorLocation && nameStart <= this.cursorLocation + 1) {
			boolean nextIsCast = this.expressionPtr > -1
					&& this.expressionStack[this.expressionPtr] instanceof CastExpression;
			m = new CompletionOnMessageSendName(null, 0, 0, nextIsCast); // positions will be set in
																			// consumeMethodInvocationName(), if that's
																			// who called us
		} else if (this.assistNode != null && this.lParenPos == this.assistNode.sourceEnd) {
			// this branch corresponds to work done in checkParemeterizedMethodName(), just the latter isn't called in
			// absence of a syntax error
	...
	private boolean checkIfAtFirstArgumentOfConstructor() {
		...
		// We try to handle the situation where lParenPos represents a argument MessageSend's left paren in the
		// AllocationExpression like
		// Foo(|null, Collections.empty(), null). Here we try to calculate the constructor's left paren and match it
		// with the
	...
	public void parseBlockStatements(AbstractMethodDeclaration md, CompilationUnitDeclaration unit) {
		...
		if ((md.bits & ASTNode.HasSyntaxErrors) != 0 && this.lastAct == ERROR_ACTION && this.assistNode != null
				&& this.assistNode.sourceEnd >= this.cursorLocation) {
	...
	public MethodDeclaration parseSomeStatements(int start, int end, int fakeBlocksCount,
			CompilationUnitDeclaration unit) {
		this.methodRecoveryActivated = true;

		initialize();

		// simulate goForMethodBody except that we don't want to balance brackets because they are not going to be
		// balanced
	...
	protected void popUntilCompletedAnnotationIfNecessary() {
		...
		int i = this.elementPtr;
		while (i > -1 && (this.elementKindStack[i] != K_BETWEEN_ANNOTATION_NAME_AND_RPAREN
	...
	protected void pushOnLabelStack(char[] label) {
		...
		if (++this.labelPtr >= stackLength) {
			System.arraycopy(this.labelStack, 0, this.labelStack = new char[stackLength + LabelStackIncrement][], 0,
					stackLength);
		}
	...
	private void pushCompletionOnMemberAccessOnExpressionStack(boolean isSuperAccess) {
		...
		if (isSuperAccess) { // considerates the fieldReference beginning at the 'super' ....
			...
			if ((fr.receiver = this.expressionStack[this.expressionPtr]).isThis()) { // fieldreference begins at the
																						// this
	...
	/**
	 * Creates a completion on method call result access node and push it on the expression stack.
	 */
	private void pushCompletionOnMethodCallResultOnExpressionStack() {
		char[] source = this.identifierStack[this.identifierPtr];
		long pos = this.identifierPositionStack[this.identifierPtr--];
		CompletionOnExpressionOfType fr = new CompletionOnExpressionOfType(
				this.expressionStack[this.expressionPtr], source, pos);
		this.assistNode = fr;
		this.lastCheckPoint = fr.sourceEnd + 1;
		this.identifierLengthPtr--;
		this.expressionStack[this.expressionPtr] = fr;
	...
	private void recordReference(NameReference nameReference) {
		if (!this.skipRecord && this.recordFrom <= nameReference.sourceStart && nameReference.sourceEnd <= this.recordTo
	...
	protected int resumeAfterRecovery() {
		...
		if (this.assistNode != null) {
			...
			/*
			 * if reached [eof] inside method body, but still inside nested type, or inside a field initializer, should
			 * continue in diet mode until the end of the method body or compilation unit
			 */
			if ((this.scanner.eofPosition >= this.cursorLocation + 1)
					&& (!(this.referenceContext instanceof CompilationUnitDeclaration)
							|| isIndirectlyInsideFieldInitialization()
							|| this.assistNodeParent instanceof FieldDeclaration
									&& !(this.assistNodeParent instanceof Initializer))) {

				/*
				 * disabled since does not handle possible field/message refs, that is, Obj[ASSIST
				 * HERE]ect.registerNatives() // consume extra tokens which were part of the qualified reference // so
				 * that the replaced source comprises them as well if (this.assistNode instanceof NameReference){ int
				 * oldEof = scanner.eofPosition; scanner.eofPosition = currentElement.topElement().sourceEnd()+1;
				 * scanner.currentPosition = this.cursorLocation+1; int token = -1; try { do { // first token might not
				 * have to be a dot if (token >= 0 || !this.completionBehindDot){ if ((token = scanner.getNextToken())
				 * != TokenNameDOT) break; } if ((token = scanner.getNextToken()) != TokenNameIdentifier) break;
				 * this.assistNode.sourceEnd = scanner.currentPosition - 1; } while (token != TokenNameEOF); } catch
				 * (InvalidInputException e){ } finally { scanner.eofPosition = oldEof; } }
				 */
	...
	protected void updateRecoveryState() {
		...
		// completionIdentifierCheck && attachOrphanCompletionNode pops various stacks to construct astNodeParent and
		// enclosingNode. This does not gel well with extended recovery.
		...
		/*
		 * check and update recovered state based on current token, this action is also performed when shifting token
		 * after recovery got activated once.
		 */
		recoveryTokenCheck();
	...
	protected CompilationUnitDeclaration endParse(int act) {
		...
		ASTNode topNode = (this.astPtr > -1) ? this.astStack[this.astPtr]
				: (this.referenceContext instanceof ASTNode) ? (ASTNode) this.referenceContext : null;
		if (topNode != null) {
			if (this.referenceContext instanceof AbstractMethodDeclaration) {
				...
				if (statement != null && isInsideBody(statement, method) && this.assistNode != null
						&& !CompletionNodeDetector.findAny(cud, this.assistNode)) {
					...
					if (method.statements == null) {
						...
						while (element != null) {
							if (element instanceof RecoveredMethod
									&& ((RecoveredMethod) element).methodDeclaration == method) {
	...
	private boolean isInsideBody(Statement statement, AbstractMethodDeclaration method) {
		...
		if (statement.sourceStart < method.bodyStart)
			return false;
		// Note that diet parsing may not have found the '{' to properly set bodyStart, so the above check is not
		// sufficient
	...
	protected FieldDeclaration createFieldDeclaration(char[] assistName, int sourceStart, int sourceEnd) {
		if (this.indexOfAssistIdentifier() < 0 || (this.currentElement instanceof RecoveredUnit
				&& ((RecoveredUnit) this.currentElement).typeCount == 0)) {
	...
	@Override
	protected RecordComponent createComponent(char[] identifierName, long namePositions, TypeReference type,
			int modifier, int declStart) {
	...
	/*
	 * To find out if the given stack has an instanceof expression at the given startIndex or at one prior to that
	 */
...
// https://bugs.eclipse.org/bugs/show_bug.cgi?id=292087
	...
	protected int actFromTokenOrSynthetic(int previousAct) {
		...
		if (this.hasError && !this.diet && newAct == ERROR_ACTION
	...
	protected boolean isInModuleStatements() {
		return isInExportsStatement() || isInOpensStatement() || isInRequiresStatement() || isInProvidesStatement()
				|| isInUsesStatement();
	}
}
