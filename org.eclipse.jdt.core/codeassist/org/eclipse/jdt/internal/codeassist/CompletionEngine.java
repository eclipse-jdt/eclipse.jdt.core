...
public final class CompletionEngine
    ...
    public class CompletionProblemFactory extends DefaultProblemFactory {
        ...
        private CategorizedProblem checkProblem(
            CategorizedProblem pb,
            ...
            parameterTypeSignature[i] =
                Signature.createCharArrayTypeSignature(
                    CharOperation.concat(
                        ...
                        CharOperation.replaceOnCopy(parameterTypeNames[i], '.', '$'), '.'
                    ), true);
        }
    ...
    public static char[] createMethodSignature(char[][] parameterPackageNames, char[][] parameterTypeNames, char[] returnPackagename, char[] returnTypeName) {
        char[] returnTypeSignature =
            ...
            : Signature.createCharArrayTypeSignature(
                CharOperation.concat(
                    returnPackagename,
                    CharOperation.replaceOnCopy(returnTypeName, '.', '$'), '.'
                ), true);
    ...
    public static char[] createNonGenericTypeSignature(char[] qualifiedPackageName, char[] qualifiedTypeName) {
        return Signature.createCharArrayTypeSignature(
            CharOperation.concat(
                ...
                CharOperation.replaceOnCopy(qualifiedTypeName, '.', '$'), '.'
            ), true);
    }
	...
	*/
    ...
    public void acceptModule(char[] moduleName) {
        ...
        if (this.assistNodeInJavadoc == 0 && this.moduleDeclaration != null && CharOperation.equals(moduleName, this.moduleDeclaration.moduleName)) {
            return;
        }
        ...
        relevance += computeRelevanceForCaseMatching(this.qualifiedCompletionToken == null
                                                     ? this.completionToken
                                                     : this.qualifiedCompletionToken, moduleName);
    ...
    public void acceptType(
        ...
        if (this.assistNodeIsExtendedType && (modifiers & ExtraCompilerModifiers.AccRecord) != 0) {
            return;
        }
    ...
    private void addExpectedType(TypeBinding type, Scope scope) {
        if (type == null || !type.isValidBinding() || type == TypeBinding.NULL || type == TypeBinding.VOID) {
            return;
        }
    ...
    public void complete(ICompilationUnit sourceUnit, int completionPosition, int pos, ITypeRoot root) {
        ...
        catch (IndexOutOfBoundsException | InvalidCursorLocation | AbortCompilation |
               CompletionNodeFound e) { // internal failure - bugs 5618
            if (DEBUG) {
    ...
    private boolean completeOnPackageVisibilityStatements(
    ...
    public void complete(IType type, char[] snippet, int position, char[][] localVariableTypeNames, char[][] localVariableNames, int[] localVariableModifiers, boolean isStatic) {
        ...
        try {
            ...
            if (typeDeclaration != null) {
               ...
               CompletionNodeFound e) { // internal failure - bugs 5618 (added with fix of 99629)
            if (DEBUG) {
                trace("Exception caught by CompletionEngine:", e); //$NON-NLS-1$
            }
        }
    ...
    private void completionOnLocalOrArgumentName(ASTNode astNode, Scope scope) {
        if (!this.requestor.isIgnored(CompletionProposal.VARIABLE_DECLARATION)) {
            ...
            if (variable instanceof CompletionOnLocalName) {
                ...
                kind = arg.isCatchArgument
                       ? InternalNamingConventions.VK_LOCAL
    ...
    private void completionOnMemberAccess(
        ASTNode astNode, ASTNode enclosingNode, Binding qualifiedBinding,
    ...
    private void completionOnMemberValueName(
        ASTNode astNode, ASTNode astNodeParent, Scope scope,
        boolean insideTypeAnnotation
    ) {
    ...
    private void pushExpectedTypesForArgumentPosition(
        ObjectVector methodsToSearchOn, int completedArgumentLength,
        Scope scope
    ) {
    ...
    private void completionOnQualifiedNameReference(
        ASTNode astNode, ASTNode enclosingNode, Binding qualifiedBinding,
        Scope scope, boolean insideTypeAnnotation
    ) {
        this.insideQualifiedReference = true;
     ...
     */
    private void internalCompletionOnQualifiedReference(
        ASTNode ref, ASTNode enclosingNode, Binding qualifiedBinding, Scope scope,
    ...
    private void completionOnQualifiedTypeReference(
        ASTNode astNode, ASTNode astNodeParent, ASTNode enclosingNode, Binding qualifiedBinding,
        Scope scope
    ) {
        this.insideQualifiedReference = true;
    ...
    private void completionOnSingleNameReference(
        ASTNode astNode, ASTNode astNodeParent, Scope scope,
        boolean insideTypeAnnotation
    ) {
        CompletionOnSingleNameReference singleNameReference = (CompletionOnSingleNameReference) astNode;
        this.completionToken = singleNameReference.token;
        SwitchStatement switchStatement = astNodeParent instanceof SwitchStatement
                                          ? (SwitchStatement) astNodeParent
                                          : null;
    ...
    private void checkForVarargExpectedTypes(ASTNode astNodeParent, Scope scope) {
        if (astNodeParent instanceof MessageSend && this.expectedTypesPtr == -1) {
            final MessageSend m = (MessageSend) astNodeParent;
...
//		}
    ...
    void findConstructors(
        ReferenceBinding currentType, TypeBinding[] argTypes, Scope scope,
            ...
            for (int f = methods.length; --f >= 0; ) {
                ...
                if (constructor.isConstructor()) {
                    ...
                    if (forAnonymousType) {
                        ...
                        if (!exactMatch) {
                            if (!isIgnored(CompletionProposal.ANONYMOUS_CLASS_CONSTRUCTOR_INVOCATION, CompletionProposal.TYPE_REF)) {
                                char[] packageName = currentType.isLocalType()
                                                     ? null
...
//				boolean inlineTagStarted = javadocTag.completeInlineTagStarted();
        ...
        for (int i = 0; i < choices.length; i++) {
            if (length <= choices[i].length && (CharOperation.prefixEquals(keyword, choices[i], false /* ignore case */)
                ...
                if (!this.requestor.isIgnored(CompletionProposal.KEYWORD)) {
                    ...
                    proposal.setReplaceRange((this.startPosition < 0)
                                             ? 0
                                             : this.startPosition - this.offset, this.endPosition - this.offset);
                    proposal.setTokenRange((this.tokenStart < 0)
                                           ? 0
                                           : this.tokenStart - this.offset, this.tokenEnd - this.offset);
                    proposal.setRelevance(relevance);
                    this.requestor.accept(proposal);
                    if (DEBUG) {
                        this.printDebug(proposal);
    ...
    private void findLocalMethods(
        ...
        if (parameterName.length > 0) {
            ...
            boostMatches = new char[][]{
                parameterName, CharOperation.concat(GET, nameForMethods),
        ...
        for (int f = methods.length; --f >= 0; ) {
            ...
            if (castedReceiver == null) {
                ...
                if (!this.isIgnored(CompletionProposal.METHOD_REF, missingElements != null) && (this.assistNodeInJavadoc & CompletionOnJavadoc.ONLY_INLINE_TAG) == 0) {
                    InternalCompletionProposal proposal = createProposal(completionOnReferenceExpressionName
                                                                         ? CompletionProposal.METHOD_NAME_REFERENCE
                                                                         : CompletionProposal.METHOD_REF, this.actualCompletionPosition);
                    proposal.setBinding(method);
                    proposal.setDeclarationSignature(getSignature(method.declaringClass));
                    proposal.setSignature(getSignature(method));
                    MethodBinding original = method.original();
    ...
    private char[] findParameterNameAtLocationFromAssistParent(ObjectVector methodsFound, Scope scope) {
        ...
        if (this.parser.assistNodeParent instanceof MessageSend) {
            final MessageSend ms = (MessageSend) this.parser.assistNodeParent;
            ...
            final MessageSend ms = (MessageSend) this.parser.assistNode;
            arguments = ms.arguments;
            ...
            final AllocationExpression ae = (AllocationExpression) this.parser.assistNodeParent;
            arguments = ae.arguments;
            if (ae.type != null && ae.type.resolvedType instanceof ReferenceBinding) {
                findConstructors(((ReferenceBinding)ae.type.resolvedType), computeTypes(arguments), scope, ae, false, null, null, null, false, methodsFound,
                    true
                );
            }
    ...
    private void findMethodDeclarations(
        ...
        if (receiverTypeMethods != null) {
            for (int i = 0; i < receiverTypeMethods.length; i++) {
                if (receiverType.isRecord() && receiverTypeMethods[i] instanceof SyntheticMethodBinding) {
                    final SyntheticMethodBinding smb = (SyntheticMethodBinding) receiverTypeMethods[i];
    ...
    private void setCompletionToken(char[][] tokens, int sourceStart, int sourceEnd, long[] sourcePositions, boolean without) {
        this.completionToken = without
                               ? CharOperation.concatWith(tokens, '.')
    ...
    private void findModules(CompletionOnModuleReference moduleReference, boolean targetted) {
        ...
        this.nameEnvironment.findModules(CharOperation.toLowerCase(this.completionToken), this, targetted
                                                                                                ? this.javaProject
                                                                                                : null);
    }
    ...
    private void findModules(CompletionOnJavadocSingleTypeReference typeReference, boolean targetted) {
        if (JavaCore.compareJavaVersions(this.sourceLevel, JavaCore.VERSION_15) >= 0) {
            ...
            try {
                ...
                this.nameEnvironment.findModules(CharOperation.toLowerCase(typeReference.token), this, targetted
                                                                                                       ? this.javaProject
                                                                                                       : null);
            }
            finally {
        ...
        while (scope != null) { // done when a COMPILATION_UNIT_SCOPE is found
            ...
            if (typeParameters != null) {
                for (int i = 0; i < typeParameters.length; i++) {
                    ...
                    relevance += computeRelevanceForExpectingType(typeParameter.type == null
                                                                  ? null
                                                                  : typeParameter.type.resolvedType);
                    relevance += computeRelevanceForQualification(false);
    ...
    private void findTypesAndPackages(char[] token, Scope scope, boolean proposeBaseTypes, boolean proposeVoidType, ObjectVector typesFound) {
        ...
        if ((proposeType || proposeConstructor) && this.unitScope != null) {
            ...
            for (int i = 0, length = types.length; i < length; i++) {
                ...
                if (this.assistNodeIsExtendedType && sourceType.isRecord()) {
                    continue next;
                }
    ...
    private void findImplementations(char[] token, Scope scope, ProvidesStatement providesStmt, int stmtIndex) {
        ...
        try {
            ...
            new SearchEngine(this.owner == null
                             ? null
                             : JavaModelManager.getJavaModelManager().getWorkingCopies(this.owner, true/*add primary WCs*/)).search(
                pattern,
                new SearchParticipant[]{SearchEngine.getDefaultSearchParticipant()},
                searchScope,
                searchRequestor,
    ...
    private void findVariableName(
        ...
        INamingRequestor namingRequestor = new INamingRequestor() {
            ...
            public void acceptNameWithPrefixAndSuffix(char[] name, boolean isFirstPrefix, boolean isFirstSuffix, int reusedCharacters) {
                accept(
                    name,
                    (isFirstPrefix ? R_NAME_FIRST_PREFIX : R_NAME_PREFIX) + (isFirstSuffix
                                                                             ? R_NAME_FIRST_SUFFIX
                                                                             : R_NAME_SUFFIX),
                    reusedCharacters
                );
    ...
    private INameEnvironment getNoCacheNameEnvironment() {
        if (this.noCacheNameEnvironment == null) {
            ...
            this.noCacheNameEnvironment = new JavaSearchNameEnvironment(this.javaProject, this.owner == null
                                                                                          ? null
    ...
    private void printDebug(CompletionProposal proposal, int tab, StringBuilder buffer) {
        ...
        buffer.append("{\n");//$NON-NLS-1$
        printDebugTab(tab, buffer);
        buffer.append("\tCompletion[").append(proposal.getCompletion() == null
                                              ? "null" .toCharArray()
                                              : proposal.getCompletion()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        printDebugTab(tab, buffer);
        buffer.append("\tDeclarationSignature[").append(proposal.getDeclarationSignature() == null
                                                        ? "null" .toCharArray()
                                                        : proposal.getDeclarationSignature()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        printDebugTab(tab, buffer);
        buffer.append("\tDeclarationKey[").append(proposal.getDeclarationKey() == null
                                                  ? "null" .toCharArray()
                                                  : proposal.getDeclarationKey()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        printDebugTab(tab, buffer);
        buffer.append("\tSignature[").append(proposal.getSignature() == null
                                             ? "null" .toCharArray()
                                             : proposal.getSignature()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        printDebugTab(tab, buffer);
        buffer.append("\tKey[").append(proposal.getKey() == null
                                       ? "null" .toCharArray()
                                       : proposal.getKey()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        printDebugTab(tab, buffer);
        buffer.append("\tName[").append(proposal.getName() == null
                                        ? "null" .toCharArray()
                                        : proposal.getName()).append("]\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
