...
public class JavaModelManager implements ISaveParticipant, IContentTypeChangeListener {
	...
	static class ZipCache {
		...
		public void clearCache(IPath path) {
			ZipFile zipFile = this.map.remove(path);
			if (zipFile != null) {
				if (JavaModelManager.ZIP_ACCESS_VERBOSE) {
					Thread currentThread = Thread.currentThread();
					trace("(" + currentThread + ") [ZipCache[" + this.owner //$NON-NLS-1$//$NON-NLS-2$
							+ "].clearCache()] ZipFile on " + zipFile.getName() + " for path: " + path); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
			Long lastModifier = this.lastModifiers.get(path);
			if (lastModifier != null) {
				this.lastModifiers.remove(lastModifier);
			}
		}
	}
	/**
