...
public class ASTConverterTest2 extends ConverterTestSetup {
	...
	public static Test suite() {
		...
		suite.addTest(new ASTConverterTest2("test0548"));
		suite.addTest(new ASTConverterTest2("test0549"));
		return suite;
	}
	/**
...
//		assertEquals("wrong id", 1, variableId);		
	 ...
	 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=58580
	 */
	public void _test0549() throws JavaModelException {
...
//		assertEquals("wrong id", 3, variableId);
		...
		variableId = variableBinding.getVariableId();
		System.out.println(variableBinding.getName() + "[" +  variableId + "]");
//		assertEquals("wrong id", 0, variableId);
...
//		assertEquals("wrong id", 1, variableId);
		...
		assertEquals("wrong size", 2, statements.size());
		Statement statement = (Statement) statements.get(0);
		assertEquals("not a variable declaration statement", ASTNode.VARIABLE_DECLARATION_STATEMENT, statement.getNodeType()); //$NON-NLS-1$
...
//		assertEquals("wrong id", 2, variableId);
		...
		node = getASTNode(unit, 0, 2);
		assertEquals("not a method declaration", ASTNode.INITIALIZER, node.getNodeType()); //$NON-NLS-1$
		Initializer initializer = (Initializer) node;
		assertEquals("wrong modifier", Modifier.STATIC, initializer.getModifiers());
		block = initializer.getBody();
		statements = block.statements();
		assertEquals("wrong size", 1, statements.size());
...
//		assertEquals("wrong id", 0, variableId);
		...
		assertEquals("not a method declaration", ASTNode.INITIALIZER, node.getNodeType()); //$NON-NLS-1$
		initializer = (Initializer) node;
		assertTrue("wrong modifier", !Modifier.isStatic(initializer.getModifiers()));
		block = initializer.getBody();
		statements = block.statements();
		assertEquals("wrong size", 1, statements.size());
		statement = (Statement) statements.get(0);
