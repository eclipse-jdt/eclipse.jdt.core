...
//		TESTS_RANGE = new int[] { -1, -1 };
	...
	/**
	 * Type declaration in a working copy test. A new type is added in the working
	 * copy only.
	 */
	public void testAddNewType() throws CoreException {
		...
		SearchPattern pattern = SearchPattern.createPattern("NewType", TYPE, DECLARATIONS,
				SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE);
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).search(pattern,
				new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, scope, this.resultCollector,
				null);
		assertSearchResults("src/wc/X.java wc.NewType [NewType]", this.resultCollector);
	}

	/*
	 * Search all type names in working copies test. (Regression test for bug 40793
	 * Primary working copies: Type search does not find type in modified CU)
	 */
	public void testAllTypeNames1() throws CoreException {
		this.workingCopy.getBuffer()
				.setContents("package wc;\n" + "public class Y {\n" + "  interface I {\n" + "  }\n" + "}");
		this.workingCopy.makeConsistent(null);
		IJavaSearchScope scope = SearchEngine
				.createJavaSearchScope(new IJavaElement[] { this.workingCopy.getParent() });
		SearchTests.SearchTypeNameRequestor requestor = new SearchTests.SearchTypeNameRequestor();
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllTypeNames(null,
				SearchPattern.R_EXACT_MATCH, null, SearchPattern.R_PATTERN_MATCH, // case insensitive
				TYPE, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
		assertSearchResults("Unexpected all type names", "wc.Y\n" + "wc.Y$I", requestor);
	}

	/*
	 * Search all type names in working copies test (without reconciling working
	 * copies). (Regression test for bug 40793 Primary working copies: Type search
	 * does not find type in modified CU)
	 */
	...
	/*
	 * Search all type names with a prefix in a primary working copy. (regression
	 * test for bug 44884 Wrong list displayed while code completion)
	 */
...
//	}
	/*
	 *
	 * Search all type names with a prefix in a primary working copy (without
	 * reconciling it). (regression test for bug 44884 Wrong list displayed while
	 * code completion)
	 *
	 * public void testAllTypeNames4() throws CoreException { ICompilationUnit wc =
	 * getCompilationUnit("/JavaSearch/wc3/X44884.java"); try {
	 * wc.becomeWorkingCopy(null); wc.getBuffer().setContents( "package wc3;\n" +
	 * "public class X44884 {\n" + "}\n" + "interface I {\n" + "}" );
	 *
	 * IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new
	 * IJavaElement[] {wc.getParent()}); SearchTests.SearchTypeNameRequestor
	 * requestor = new SearchTests.SearchTypeNameRequestor(); new
	 * SearchEngine().searchAllTypeNames( "wc3".toCharArray(),
	 * SearchPattern.R_EXACT_MATCH, "X".toCharArray(), SearchPattern.R_PREFIX_MATCH,
	 * // case insensitive TYPE, scope, requestor,
	 * IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null ); assertSearchResults(
	 * "Unexpected all type names", "wc3.X44884", requestor); } finally {
	 * wc.discardWorkingCopy(); } }
	 */

	/**
	 * Bug 99915: [search] Open Type: not yet saved types not found if case-sensitve
	 * name is entered
	 *
	...
	public void testAllTypeNamesBug99915() throws CoreException {
		...
		SearchTests.SearchTypeNameRequestor requestor = new SearchTests.SearchTypeNameRequestor();
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllTypeNames(null,
				SearchPattern.R_EXACT_MATCH, "A*".toCharArray(), SearchPattern.R_PATTERN_MATCH, // case insensitive
				TYPE, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
	...
	public void testAllTypeNamesBug98684() throws CoreException {
		try {
			...
			new SearchEngine(this.workingCopies).searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH,
					"A".toCharArray(), SearchPattern.R_PREFIX_MATCH, TYPE, scope, requestor,
	...
	/**
	 * Declaration of referenced types test. (Regression test for bug 5355 search:
	 * NPE in searchDeclarationsOfReferencedTypes )
	 */
	public void testDeclarationOfReferencedTypes() throws CoreException {
		IMethod method = this.workingCopy.getType("X")
				.createMethod("public void foo() {\n" + "  X x = new X();\n" + "}", null, true, null);
		// JavaSearchResultCollector resultCollector = new JavaSearchResultCollector();
	...
	/**
	 * Type declaration in a working copy test. A type is moved from one working
	 * copy to another.
	 */
	public void testMoveType() throws CoreException {

		// move type X from working copy in one package to a working copy in another
		// package
		...
		try {
			...
			IJavaSearchScope scope1 = SearchEngine
					.createJavaSearchScope(new IJavaElement[] { workingCopy1.getParent() });
			// JavaSearchResultCollector resultCollector = new JavaSearchResultCollector();

			SearchPattern pattern = SearchPattern.createPattern("X", TYPE, DECLARATIONS,
					SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE);
			searchEngine.search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, scope1,
					this.resultCollector, null);
			assertEquals("", this.resultCollector.toString());

			// type X should be visible in new package
			IJavaSearchScope scope2 = SearchEngine
					.createJavaSearchScope(new IJavaElement[] { workingCopy2.getParent() });
			this.resultCollector = new JavaSearchResultCollector();
			searchEngine.search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, scope2,
					this.resultCollector, null);
	...
	/**
	 * Type declaration in a working copy test. A type is removed from the working
	 * copy only.
	 */
	public void testRemoveType() throws CoreException {
		...
		new SearchEngine().search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() },
				scope, this.resultCollector, null);
		assertSearchResults("src/wc/X.java wc.X [X]", this.resultCollector);

	}

	/*
	 * Search all method names in working copies test (without reconciling working
	 * copies).
	 */
	public void testBug478042_dirtyWC_001() throws CoreException {
		this.workingCopy.getBuffer()
				.setContents("package p478042;\n" + "public class AllMethodDeclarations01 {\n"
		...
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllMethodNames("p478042".toCharArray(),
				SearchPattern.R_EXACT_MATCH, // package
				...
				"foo".toCharArray(), SearchPattern.R_PREFIX_MATCH, scope, requestor,
				IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
	...
	public void testBug478042_dirtyWC_004() throws CoreException {
		...
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllMethodNames(
				"p478042.AllMethod*".toCharArray(), SearchPattern.R_PATTERN_MATCH, "foo".toCharArray(),
				SearchPattern.R_PREFIX_MATCH, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
		assertSearchResults("/JavaSearch/src/wc/X.java char p478042.AllMethodDeclarations01.foo03(Object o,String s)\n"
				+ "/JavaSearch/src/wc/X.java int p478042.AllMethodDeclarations01.foo02(Object o)\n"
				+ "/JavaSearch/src/wc/X.java void p478042.AllMethodDeclarations01.foo01()", requestor);
	}
	...
	public void testBug483650_dirtyWC_002() throws CoreException {
		...
		SearchTests.SearchMethodNameRequestor requestor = new SearchTests.SearchMethodNameRequestor();
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllMethodNames(
				"p478042.AllMethod*.Nested".toCharArray(), SearchPattern.R_PATTERN_MATCH, "foo".toCharArray(),
				SearchPattern.R_PREFIX_MATCH, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
		assertSearchResults(
				"/JavaSearch/src/wc/X.java char p478042.AllMethodDeclarations01.Nested.foo03(Object o,String s)\n"
						+ "/JavaSearch/src/wc/X.java int p478042.AllMethodDeclarations01.Nested.foo02(Object o)\n"
	...
	public void testBug483650_dirtyWC_003() throws CoreException {
		...
		SearchTests.SearchMethodNameRequestor requestor = new SearchTests.SearchMethodNameRequestor();
		new SearchEngine(new ICompilationUnit[] { this.workingCopy }).searchAllMethodNames(
				"p478042.AllMethod*.Inner".toCharArray(), SearchPattern.R_PATTERN_MATCH, "foo".toCharArray(),
				SearchPattern.R_PREFIX_MATCH, scope, requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
		assertSearchResults(
				"/JavaSearch/src/wc/X.java char p478042.AllMethodDeclarations01.Nested.Inner.foo03(Object o,String s)\n"
						+ "/JavaSearch/src/wc/X.java int p478042.AllMethodDeclarations01.Nested.Inner.foo02(Object o)\n"
