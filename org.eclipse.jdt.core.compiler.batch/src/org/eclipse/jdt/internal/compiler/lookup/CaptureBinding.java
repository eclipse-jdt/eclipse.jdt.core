		...
		switch (this.wildcard.boundKind) {
			case Wildcard.EXTENDS :
				...
				if (originalWildcardBound.isInterface()) {
					...
					if (this.superclass.id != TypeIds.T_JavaLangObject && substitutedVariableInterfaces != Binding.NO_SUPERINTERFACES) {
						int length = substitutedVariableInterfaces.length;
						...
						if (superTypes != null) {
							int nonInterfaceCount = 0;
							for (int i = 0; i < superTypes.length; i++) {
								if (!superTypes[i].isInterface()) {
									nonInterfaceCount++;
								}
							}
							substitutedVariableInterfaces = new ReferenceBinding[superTypes.length - nonInterfaceCount];
							if (substitutedVariableInterfaces.length != 0) {
								int offset = 0;
								for (int i = 0; i < superTypes.length; i++) {
									ReferenceBinding current = superTypes[i];
									if (current.isInterface()) {
										substitutedVariableInterfaces[i - offset] = superTypes[i];
									} else {
										offset++;
									}
								}
							} else {
								substitutedVariableInterfaces = Binding.NO_SUPERINTERFACES;
							}
						} else {
							substitutedVariableInterfaces = Binding.NO_SUPERINTERFACES;
						}
