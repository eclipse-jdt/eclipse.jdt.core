...
public class InferenceContext18 {
	...
	*/
	 ...
	 * @param maySkipSuperBound if true, then α :> β bounds will not be treated as a dependency from α to β
	...
	Map<InferenceVariable,Set<InferenceVariable>> collectDependencies(BoundSet bounds, boolean maySkipSuperBound, boolean[] hasSkippedSuperBound) {
		...
		// each inference variable depends on itself:
		...
		for (TypeBound typeBound : bounds.flatten()) {
			// by construction typeBound has one of the forms:
			// α = T
			// α <: T
			// T = α  -- encoded as α = T
			// T <: α -- encoded as α :> T
			for (int i=0; i<2; i++) {
				...
				if (typeBound.right instanceof InferenceVariable ivRight) {
					// if T = is ivar then repeat in inverse direction
		...
		for (Entry<ParameterizedTypeBinding, ParameterizedTypeBinding> capEntry : bounds.captures.entrySet()) {
			...
			if (allIVs.isEmpty()) continue;
			for (InferenceVariable left : alphas) {
				Set<InferenceVariable> deps = dependsOn.computeIfAbsent(left, iv -> new LinkedHashSet<>());
				deps.addAll(allIVs);
			}
		}
		// transitive closure by fix point computation:
		boolean hasChange;
		do {
			hasChange = false;
			for (Entry<InferenceVariable, Set<InferenceVariable>> deps : dependsOn.entrySet()) {
				Set<InferenceVariable> rights = new HashSet<>(deps.getValue());
				for (InferenceVariable right : rights) {
					if (TypeBinding.equalsEquals(right, deps.getKey())) continue;
					Set<InferenceVariable> rightDeps = dependsOn.get(right);
					if (rightDeps != null)
						hasChange |= deps.getValue().addAll(rightDeps);
				}
			}
		} while (hasChange);
		return dependsOn;
