...
private FlowInfo analyzeFieldInitializations(ClassScope classScope, InitializationFlowContext initializerFlowContext, FlowInfo prologueInfo) {
    ...
    */
	...
	FlowInfo flowInfo = prologueInfo.unconditionalCopy().discardNonFieldInitializations(limit);
	...
	if (typeDeclaration.initializerScope.inPrimaryAnalysis()) {
		...
		isCloseable = typeDeclaration.binding.hasTypeBit(TypeIds.BitAutoCloseable|TypeIds.BitCloseable);
	}

	IErrorHandlingPolicy oldPolicy = adoptUniqueErrorHandlingPolicy(typeDeclaration.initializerScope.problemReporter());
	try {
	...
	FlowInfo epilogueEntryInfo = prologueInfo.copy().addInitializationsFrom(flowInfo);
	epilogueEntryInfo.addNullInfoFrom(prologueInfo);
	epilogueEntryInfo.setReachMode(flowInfo.reachMode());
	return epilogueEntryInfo;
}

private IErrorHandlingPolicy adoptUniqueErrorHandlingPolicy(ProblemReporter problemReporter) {
	IErrorHandlingPolicy oldPolicy = problemReporter.policy;
	return problemReporter.switchErrorHandlingPolicy(new IErrorHandlingPolicy () {
			@Override
			public boolean stopOnFirstError() {
				return oldPolicy.stopOnFirstError();
			}
			@Override
			public boolean proceedOnErrors(){
				return oldPolicy.proceedOnErrors();
			}
			@Override
			public boolean ignoreAllErrors() {
				return oldPolicy.ignoreAllErrors();
			}
			@Override
...
public void analyseCode(ClassScope classScope, InitializationFlowContext initializerFlowContext, FlowInfo flowInfo) {
	...
	try {
		ExceptionHandlingFlowContext constructorContext = new ExceptionHandlingFlowContext(
				initializerFlowContext.parent, this, this.binding.thrownExceptions, initializerFlowContext, this.scope,
				FlowInfo.DEAD_END);
			...
			for (Statement stat : this.statements) {
				...
				if (stat instanceof ExplicitConstructorCall) {
					...
					if (constructorCall.accessMode == ExplicitConstructorCall.This) {
						...
						if ((constructorCall.bits & IsReachable) != 0) { // don't pollute the field initializations if they are unreachable in this universe.
