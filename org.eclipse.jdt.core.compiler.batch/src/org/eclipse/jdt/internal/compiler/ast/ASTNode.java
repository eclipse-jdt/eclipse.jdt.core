			...
			TypeReference typeRef, TypeBinding existingType)
	{
		...
		if (se8nullBits == TagBits.AnnotationNullMASK) {
			...
			se8Annotations = scope.environment().filterNullTypeAnnotations(se8Annotations);
					...
					&& typeRef.dimensions() > 0  // in this case moving from SE7 to SE8 position applies it to the array contents!
					&& scope.hasDefaultNullnessForType(typeRef.resolvedType, Binding.DefaultLocationArrayContents, typeRef.sourceStart)) {
