...
public String getBindingMethodName() {
	...
	public TypeBinding resolveType(BlockScope scope) {
		...
		if (!originalLhsType.isBoxedPrimitiveType() && !originalLhsType.isBaseType() && originalLhsType.id != T_JavaLangString){
			...
			if (mb2 != null){
				...
				if(isOverloadedArray(this.lhs)){
					...
					typeCompoundOverloadOperator = ((Reference)this.lhs).resolveTypeCompoundOverloadOperator(scope, this.appropriateMethodForOverload.returnType);
					if( typeCompoundOverloadOperator == null){
						//No put
						return null;
					}else{
						if(!typeCompoundOverloadOperator.equals(TypeBinding.VOID)){
							scope.problemReporter().typeMismatchError(typeCompoundOverloadOperator, TypeBinding.VOID, this.lhs, null);
							return null;
						}
					}
				}
		...
		if(mb2 != null && mb2.isValidBinding()){
			...
			if(typeCompoundOverloadOperator != null){
				return typeCompoundOverloadOperator;
			}
			return this.resolvedType = mb2.returnType;
		}
