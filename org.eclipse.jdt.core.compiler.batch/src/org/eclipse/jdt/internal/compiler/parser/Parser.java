...
protected void consumeConstructorDeclaration() {
	...
	*/
	...
	if (!this.options.ignoreMethodBodies) {
		...
		if (length > 0 || !this.diet || insideFieldInitializer) // if diet mode then constructor body will be added elsewhere.
			cd.buildBody(this.astStack, this.astPtr + 1, length, this.options);
	}

	if (!(this.diet && this.dietInt == 0)
			&& cd.statements != null
			&& (cd.statements.length == 0 || (cd.statements.length == 1 && cd.statements[0].isImplicitSuper()))
...
protected void consumeRule(int act) {
 ...
 consumeMethodDeclaration(false, false); 			break;
    ...
    case 539 : if (DEBUG) { System.out.println("LambdaParameters ::= UNDERSCORE NestedLambda"); }  //$NON-NLS-1$
    ...
    case 548 : if (DEBUG) { System.out.println("TypeElidedFormalParameter ::= UNDERSCORE"); }  //$NON-NLS-1$
		    consumeBracketedTypeElidedUnderscoreLambdaParameter(); 			break;

    case 549 : if (DEBUG) { System.out.println("LambdaBody ::= Expression"); }  //$NON-NLS-1$
    ...
    case 621 : if (DEBUG) { System.out.println("CastedLambdaExpression ::= PushLPAREN Name..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithGenericsArray(); 			break;

    case 622 : if (DEBUG) { System.out.println("CastedLambdaExpression ::= PushLPAREN Name..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithQualifiedGenericsArray(); 			break;

    case 623 : if (DEBUG) { System.out.println("CastedLambdaExpression ::= PushLPAREN Name PushRPAREN..."); }  //$NON-NLS-1$
		    consumeCastExpressionLL1(); 			break;

    case 624 : if (DEBUG) { System.out.println("CastedLambdaExpression ::= BeginIntersectionCast..."); }  //$NON-NLS-1$
		    consumeCastExpressionLL1WithBounds(); 			break;

    case 625 : if (DEBUG) { System.out.println("CastedLambdaExpression ::= PushLPAREN Name Dims..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithNameArray(); 			break;
    ...
    case 713 : if (DEBUG) { System.out.println("PermittedTypesopt ::= RestrictedIdentifierpermits..."); }  //$NON-NLS-1$
    ...
    case 755 : if (DEBUG) { System.out.println("SingleModuleImportDeclaration ::=..."); }  //$NON-NLS-1$
		    consumeImportDeclaration(); 			break;

    case 756 : if (DEBUG) { System.out.println("SingleModuleImportDeclarationName ::= import module Name"); }  //$NON-NLS-1$
...
public MethodDeclaration convertToMethodDeclaration(ConstructorDeclaration c, CompilationResult compilationResult) {
	...
	int length = c.statements != null ? c.statements.length : 0;
	if (length > 0) {
		if (c.statements[0] instanceof ExplicitConstructorCall) {
			System.arraycopy(c.statements, 1, m.statements = new Statement[length - 1], 0, length - 1);
		} else if (c.statements[length - 1] instanceof ExplicitConstructorCall) {
			System.arraycopy(c.statements, 0, m.statements = new Statement[length - 1], 0, length - 1);
		}
	} else {
...
public void parse(ConstructorDeclaration cd, CompilationUnitDeclaration unit, boolean recordLineSeparator) {
	...
	int length = this.astLengthPtr > -1 ? this.astLengthStack[this.astLengthPtr--] : 0;
	this.astPtr -= length;
	if (!this.options.ignoreMethodBodies) {
		cd.buildBody(this.astStack, this.astPtr + 1, length, this.options);
	}
