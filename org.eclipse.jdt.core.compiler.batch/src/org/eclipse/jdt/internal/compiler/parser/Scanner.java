...
public char[] getCurrentIdentifierSource() { // has side effects in overrides
	return peekCurrentIdentifierSource();
}

public char[] peekCurrentIdentifierSource() { // side effect free
...
public int getNextToken() throws InvalidInputException {
	...
	token = getNextToken0();
	updateCase(token);
	if (token == TokenNameNotAToken /* FIX */) {
...
private VanguardScanner getNewVanguardScanner() {
	...
	vs.resetTo(this.startPosition, this.eofPosition - 1);
	return vs;
}
...
protected final boolean maybeAtEnhForRecordPattern() {
	return this.lookBack[1] == TokenNamefor;
}
