...
public class GenerateParserScript {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		String terminalTokensText = new String(Files.readAllBytes(terminalTokensFile.toPath())).replace("\r\n",  "\n");
		{
			...
			int end = terminalTokensText.indexOf(endTag);
			assertTrue(end >= 0);
			String terminalTokensProlog = terminalTokensText.substring(0, start);
			String terminalTokensEpilog = terminalTokensText.substring(end);
			...
			javasymText = javasymText.replace("}", "");
			javasymText = javasymText.replaceAll("=\\s*(\\d+)", "($1)");
			
			Files.write(terminalTokensFile.toPath(), (terminalTokensProlog + "\t\t\t\t\t\t\t" + javasymText + terminalTokensEpilog).getBytes());
		}
